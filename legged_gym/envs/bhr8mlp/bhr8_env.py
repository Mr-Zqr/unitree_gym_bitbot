...
# used for training
class BHR8MLPRobot(LeggedRobot):
    ...
    def _init_buffers(self):
        super()._init_buffers()
        self.obs_history = deque(maxlen=self.cfg.env.a_stack_length)
        for _ in range(self.cfg.env.a_stack_length):
            self.obs_history.append(torch.zeros(self.num_envs, self.cfg.env.a_obs_unit, device=self.device))
        self._init_foot()
    ...
    def compute_observations(self):
        ...
        self.obs_history.append(obs_buf_unit)
        obs_buf_all = torch.stack([self.obs_history[i] for i in range(self.obs_history.maxlen)], dim=1)
        obs_buf_all = obs_buf_all.view(self.num_envs, -1)
        self.obs_buf = obs_buf_all.clone()
        # add noise if needed
        if self.add_noise:
