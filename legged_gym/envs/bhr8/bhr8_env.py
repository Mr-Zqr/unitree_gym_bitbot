...
class BHR8Robot(LeggedRobot):
    ...
    def _post_physics_step_callback(self):
        ...
        self._in_place_flag = 0*(torch.norm(self.commands[:, :2], dim=-1) < 0.1)
    ...
    def _reward_no_fly(self):
        ...
        in_contact = torch.sum(1.*contacts, dim=1)>=1
        res = 1.*in_contact
        res[self._in_place_flag] = 0
        return res
    ...
    def _reward_stand_still_exbody(self):
        ...
        rew = torch.exp(- 0.3*dof_vel_error) * torch.exp(- dof_pos_error) 
        rew[~self._in_place_flag] = 0
        return rew
